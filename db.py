...
def insert_user(uid, name, email, phone=None, status="active",
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT OR REPLACE INTO users (uid, name, email, phone, status, created_at, updated_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
def insert_device(device_id, home_id, device_name, device_type=None,
    ...
    cursor.execute("""
        ...
        (id, home_id, device_name, device_type, device_status, last_seen, created_at)
        VALUES (?, ?, ?, ?, ?, ?, ?)
...
def update_device_status(device_id, status, last_seen=None):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        UPDATE devices SET device_status = ?, last_seen = ? WHERE id = ?
...
def insert_model(model_name, version, local_path, cloud_path=None,
    ...
    if existing:
        cursor.execute("""
            ...
            WHERE id = ?
        """, (local_path, cloud_path, active, downloaded_at, existing["id"]))
    else:
        cursor.execute("""
            INSERT INTO models (model_name, version, local_path, cloud_path,
...
def get_active_model(model_name):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        SELECT * FROM models WHERE model_name = ? AND active = 1
...
def insert_blockchain_audit(batch_id, action, status, message=None):
    ...
    cursor = conn.cursor()
    cursor.execute("""
        INSERT INTO blockchain_audit (batch_id, action, status, message, created_at)
        VALUES (?, ?, ?, ?, ?)
